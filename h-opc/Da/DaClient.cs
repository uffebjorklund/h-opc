...
namespace Hylasoft.Opc.Da
{
    ...
    public partial class DaClient : IClient<DaNode>
    {
        ...
        /// <param name="interval">custom interval, have to be over 100 to be used</param>
        public void Monitor(List<string> tags, Action<OpcDa.ItemValueResult[], Action> callback, int interval = -1)
        {
            var _interval = DefaultMonitorInterval;
            if (interval > 100) {
                _interval = interval;
            }
            var subItem = new OpcDa.SubscriptionState
            ...
            sub.AddItems(tags.Select(p => new OpcDa.Item { ItemName = p }).ToArray());
            sub.SetEnabled(true);
        }
